...
//    public static ArrayList<Bag> allBags = new ArrayList<>();
//    public static ArrayList<Box> allBox = new ArrayList<>();
//
...
//                Bag tempBag = new Bag(bag.type, bag.X, bag.Y, bag.cost, 1);
////                System.out.println(tempBag.cost);
//                allBags.add(tempBag);
//            }
//        }
...
//        }
//        for (int i = 0; i < bagCount; i++) {
//            cplex.addLe(L[i],allBags.get(i).X);
//            cplex.addLe(W[i],allBags.get(i).Y);
//            cplex.addLe(H[i],Math.min(allBags.get(i).X,allBags.get(i).Y));
//        }
//        IloLinearNumExpr obj = cplex.linearNumExpr();
//        for (int j = 0; j < bagCount; j++) {
//            obj.addTerm(allBags.get(j).cost, n[j]);
////            System.out.println(allBags.get(j).cost);
//        }
//        cplex.addMinimize(obj);
////        cplex.use(new Callback(L,W,H,n, cplex));
////        cplex.use(new LazyCallback(L,W,H,n,cplex));
////        System.exit(0);
...
//            for (int i = 0; i < boxCount; i++) {
//                Position position = new Position(allBox.get(i).type, xx[i], yy[i], zz[i], lx[i], ly[i], lz[i]);
...
//                }
//            }
////            PrintStream prs = new PrintStream("result\\resultB_C" + url + "_" + suanli + ".txt");
////            System.setOut(prs);
...
//        IloNumVar[] L,W,H,n;
//        IloCplex cplex;
//        int boxCount = allBox.size(), bagCount = allBags.size();
//
//        public Callback(IloNumVar[] l, IloNumVar[]w, IloNumVar[] h, IloNumVar[]n,IloCplex cplex) {
//            L = l;
//            W = w;
//            H = h;
//            this.n=n;
...
//            double[] lSol = getValues(L);
//            double[] wSol = getValues(W);
//            double[] hSol = getValues(H);
//            for (int i = 0; i < bagCount; i++) {
//                if (getValue(n[i])<0.5) continue;
//                if (lSol[i]+hSol[i]>=allBags.get(i).X){
//                    addLocal(cplex.le(cplex.sum(L[i],H[i]),allBags.get(i).X));
//                }
//                if (wSol[i]+hSol[i]>=allBags.get(i).Y){
//                    addLocal(cplex.le(cplex.sum(W[i],H[i]),allBags.get(i).Y));
//                }
//            }
//
//        }
//    }
//    public static class LazyCallback1 extends IloCplex.LazyConstraintCallback{
//        IloNumVar[] L,W,H,n;
//        IloCplex cplex;
//        int boxCount = allBox.size(), bagCount = allBags.size();
//
//        public LazyCallback1(IloNumVar[] l, IloNumVar[]w, IloNumVar[] h, IloNumVar[]n,IloCplex cplex) {
//            L = l;
//            W = w;
//            H = h;
//            this.n=n;
...
//                if (getValue(n[i])<0.5) continue;
//                if (lSol[i]+hSol[i]>=allBags.get(i).X){
//                    add(cplex.le(cplex.sum(L[i],H[i]),allBags.get(i).X));
//                }
//                if (wSol[i]+hSol[i]>=allBags.get(i).Y){
//                    add(cplex.le(cplex.sum(W[i],H[i]),allBags.get(i).Y));
//                }
//            }
//        }
//    }
////    public static class LazyCallback2 extends IloCplex.LazyConstraintCallback{
////        IloNumVar
////    }
